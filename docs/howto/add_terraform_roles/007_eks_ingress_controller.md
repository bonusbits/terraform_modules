...
```cp -R $BB_ORCHESTRATOR_PATH/terraform/environments/examples/eks_ingress_controller $BB_ORCHESTRATOR_PATH/terraform/environments/my_app/eks_ingress_controller```

#### TFVars File
1. Copy example eks_cluster.tfvars file to our environment folder/s. i.e. myapp-dev-use1
```cp -R $BB_ORCHESTRATOR_PATH/vars/terraform/examples/eks_ingress_controller.tfvars $BB_ORCHESTRATOR_PATH/vars/terraform/myapp-dev-use1/eks_ingress_controller.tfvars```
