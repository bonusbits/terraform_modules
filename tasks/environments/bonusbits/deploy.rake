    ...
    task :stack do
      ...
      $project_vars['terraform']['roles'].each do |role|
        ...
        when 'bastion'
          ...
          Orchestrator::ConsoleOutputs.debug_message('bonusbits:deploy:stack - setup cased to bastion')
...
desc 'Alias (bonusbits:deploy:stack)'
task bb_up: %w[bonusbits:deploy:stack]
